...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    if (!flightId) return;
    try {
      const listRes = await axios.get(`http://localhost:3001/api/admin/queue/${flightId}`);
      setQueueList(listRes.data);
      const metricRes = await axios.get(`http://localhost:3001/api/admin/metrics/${flightId}`);
  ...
  const handleCallNext = async () => {
    try {
      const res = await axios.post('http://localhost:3001/api/admin/call-next', { flightId, counterId: "Counter-1" });
  ...
  const handleReset = async () => {
    ...
    try {
      await axios.post('http://localhost:3001/api/admin/reset');
  ...
  const handleSimulate = async () => {
    try {
      ...
      await axios.post('http://localhost:3001/api/admin/simulate', { flightId, count: 5 });
      toast.dismiss();
  ...
  const handleCreateFlight = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/api/admin/flights', newFlight);
