...
app.post('/api/admin/flights', async (req, res) => {
  ...
  try {
    const flight = await prisma.flight.create({
      data: { flightCode, destination, departureTime: new Date(departureTime) }
    });
    res.json(flight);
  } catch (error) {
    res.status(500).json({ error: "Failed to register flight" });
  }
});
...
app.post('/api/queue/join', async (req, res) => {
  try {
    const { userId, flightId, ticketClass, isSpecialNeeds } = req.body;
...
// Check Active Session (Persistence)
app.get('/api/queue/active/:userId', async (req, res) => {
  try {
...
// Get Queue (Serving + Waiting)
app.get('/api/admin/queue/:flightId', async (req, res) => {
...
// Call Next Passenger (Smart Logic)
...
// Metrics
app.get('/api/admin/metrics/:flightId', async (req, res) => {
  try {
    const { flightId } = req.params;
